...
def apply_lora(model, rank=4, alpha=16, use_bayesian=False):
    if use_bayesian:
        assign_linearlora = partial(BayesianLinearWithLoRA, rank=rank, alpha=alpha)
    else:
        assign_linearlora = partial(LinearWithLoRA, rank=rank, alpha=alpha)

    for i, layer in enumerate(model.fc):
        if isinstance(layer, nn.Linear):
            model.fc[i] = assign_linearlora(layer)
